...
{<<<}
const
  codemax     =  100000(*8650*); (* increased this for testing [sam] *)
  pcmax       =  200000(*17500*);
  ...
  overb       =   70000(*13820*);
  overm       = 1070000(*18000*); (* increased this for testing [sam] *)
  maxstr      = 1070001(*18001*); (* increased this for testing [sam] *)
...
{<<<}
var
  code        : array[0..codemax] of   (* the program *)
...
var  
  ...
  procedure assemble; (*translate symbolic code into machine code and store*)
   ...
   var name :alfa;  (*b :boolean;*)  r :real;  s :settype;
     ...
     begin 
       while (ch<>'l') and not eoln(prd) do read(prd,ch);
       read(prd,x); 
       lookup(x)
     end;(*labelsearch*)
     {>>>}
  ...
  begin
    ...
    case op of  (* get parameters p,q *)
      ...
      {<<<}
      17,18,19, 20,21,22: begin case ch of
                          'a': ; (*p = 0*)
                          'i': p := 1;
                          'r': p := 2;
                          'b': p := 3;
  ...
  begin
    ...
    case op of
      ...
      70,71,72,73,74,2 (*str*):
                 begin  store[base(p)+q] := store[sp];
                  sp := sp-1
                 end;
      {>>>}
      {<<<}
      75,76,77,78,79,3 (*sro*):
                 begin  store[q] := store[sp];
                  sp := sp-1
                 end;
      {>>>}
      {<<<}
      ...
      90,91,92,10 (*inc*): store[sp].vi := store[sp].vi+q;
      ...
      {<<<}
      96,97, 26 (*chk*): if (store[sp].vi < store[q-1].vi) or
      ...
      100,101,102,57 (*dec*): store[sp].vi := store[sp].vi-q;
