...
TYPE
    filed_history_t = RECORD CASE boolean OF
		      true: (num_recs: integer;
			    recs: ARRAY [ 1 .. recs_per_file_rec$ ]
					OF history_t);
		      false: (dummy: PACKED ARRAY [ 1 .. bytes_per_file_rec$ ]
					OF char);
		      END;
...
[EXTERNAL] FUNCTION lib$get_foreign( %DESCR result,user_prompt:string;
	%REF out_len:word;
	%REF force_prompt:integer):integer;
EXTERN;

TYPE
unsafe_file_t = [unsafe]file OF block;
fab_ptr_t = ^FAB$TYPE;
nam_ptr_t = ^NAM$TYPE;

FUNCTION PAS$FAB (VAR F: unsafe_file_t) : fab_ptr_t;
  EXTERN;
...
VAR
  ql_undefsym,ql_dbldefsym,ql_sizeclash,ql_fatalerr,
  ql_sctovrlap : integer;

  firstfile : boolean;
  termchar : char;
  return_code,smax,checksum,pass,opos,bpos : integer;
  o : objrec;
...
IF english THEN
  ...
  IF sectbase[12] <> 0 THEN
    BEGIN
    writeln (log_file, 'Size of error messages   (12)  = ',
	      sectbase[12]:8, ' bytes');
    total := total + sectbase[12];
    END;
  IF sectbase[13] <> 0 THEN
    BEGIN
    writeln (log_file, 'Size of code & constants (13)  = ',
	      sectbase[13]:8, ' bytes');
    total := total + sectbase[13];
    END;
  IF sectbase[14] <> 0 THEN
    BEGIN
    writeln (log_file, 'Size of diagnostic block (14)  = ',
	      sectbase[14]:8, ' bytes');
    total := total + sectbase[14];
    END;
  IF sectbase[15] <> 0 THEN
    BEGIN
    writeln (log_file, 'Size of global variables (15)  = ',
	      sectbase[15]:8, ' bytes');
    total := total + sectbase[15];
    END;
  writeln (log_file, 'Total size                     = ',
	      total:8, ' bytes');
  END
...
ELSE FOR i:=0 TO 15 DO
  IF sectbase[i] <> 0 THEN
    BEGIN
    write(log_file, 'Section ',i:2,' Start ',hex(baseaddr[i],6,6),' Length ',
					hex(sectbase[i],6,6));
...
close(log_file);
END; { of PROCEDURE close_log_file }
...
VAR
  ...
  END; { of FUNCTION clash }


BEGIN { of PROCEDURE overlap check }
FOR i := 0 TO 14 DO
  FOR j := i+1 TO 15 DO
    IF clash(i,j) THEN
      BEGIN
...
END; { of PROCEDURE overlap check }
...
IF log THEN writeln (log_file, 'Undefined symbols:-');
FOR i := 0 TO max_hash DO
  IF hash_table[i] <> nil THEN
    BEGIN
    s_ptr := hash_table[i];
    ...
    REPEAT
      IF NOT s_ptr^.sdef THEN
	BEGIN
	writeln('''',s_ptr^.sname,''' first referenced in module ''',
		s_ptr^.mname,'''');
	IF log THEN
	  writeln(log_file, '''',s_ptr^.sname,''' first referenced in module ''',
		  s_ptr^.mname,'''');
	s_ptr^.ssect := -1;
	s_ptr^.saddr := %X'FAFF';
	END;
...
BEGIN
FOR i := 1 TO 10 DO sn[i] := chr(gbyte);
getnam := sn;
END;  { of PROCEDURE getnam }
...
BEGIN
i := 0;
FOR j := 1 TO 4 DO i := mvl(i) + gbyte;
getint := i;
END; { of PROCEDURE get_int }
...
BEGIN
IF download THEN writeln(tgtfile);
IF out THEN writeln(sfile);
END; { of PROCEDURE senddfnewline }

{-------------------------------------------------------------------------}
PROCEDURE sendsform(VAR s:string);

BEGIN
IF out THEN write(sfile,s);
IF download THEN write(tgtfile,s);
END; { of PROCEDURE sendsform }
...
{***************************************************************************}
BEGIN { main code of PROGRAM }

ql_dbldefsym := %X'08008004';
ql_undefsym := %X'0800800C';
ql_fatalerr := %X'08008014';
ql_sizeclash := %X'0800801A';
ql_sctovrlap := 0;
return_code := 0;
...
IF chat OR debug THEN writeln( prompt );
prompt := prompt+' >';
fil_len := -1;
pflag := 0;
...
IF colonpos > dirend THEN dirend:=colonpos;
IF dirend <> 0 THEN
...
IF chat OR debug THEN writeln( 'File given is ', cmdname );
...
REPEAT
  ...
  IF termchar = '=' THEN
    BEGIN
    IF firstfile THEN cmdroot := f
    ELSE writeln('You can''t put an ''='' THERE!');
    END
...
IF chat OR debug OR (NOT quiet) THEN
  ...
  IF log THEN writeln (log_file, numsymbols:5,' in symbol table');
  END;
...
FOR i:=0 TO 15 DO
  ...
  IF sectbase[i] <> 0 THEN
    BEGIN
    IF NOT english THEN
      write('Section ',i:2,' Start ',hex(basepos,6,6),' Length ',
				      hex(sectbase[i],6,6));
    baseaddr[i]:=basepos;
    basepos:=basepos+sectbase[i];
    IF NOT english THEN writeln(' Finish  ',hex(basepos,6,6));
    END;
  END;
...
IF english THEN
  ...
  IF sectbase[9] <> 0 THEN
    BEGIN
    writeln ('Size of HELP              (9)  = ',
	      sectbase[9]:8, ' bytes');
    total := total + sectbase[9];
    END;
  IF sectbase[12] <> 0 THEN
    BEGIN
    writeln ('Size of error messages   (12)  = ',
	      sectbase[12]:8, ' bytes');
    total := total + sectbase[12];
    END;
  IF sectbase[13] <> 0 THEN
    BEGIN
    writeln ('Size of code & constants (13)  = ',
	      sectbase[13]:8, ' bytes');
    total := total + sectbase[13];
    END;
  IF sectbase[14] <> 0 THEN
    BEGIN
    writeln ('Size of diagnostic block (14)  = ',
	      sectbase[14]:8, ' bytes');
    total := total + sectbase[14];
    END;
  IF sectbase[15] <> 0 THEN
    BEGIN
    writeln ('Size of global variables (15)  = ',
	      sectbase[15]:8, ' bytes');
    total := total + sectbase[15];
    END;
  writeln ('Total size                     = ',
...
IF undefsym<>0 THEN
  ...
  IF log THEN writeln (log_file, 'Number of undefined symbols:- ',undefsym);
  check_undef;
  END;
...
IF modules OR out OR download THEN
  ...
  IF modules THEN close(mod_file);
  closeoutput;
  END;

end_pass_2 := current_time;

IF history THEN disp_history;
end_his_gen:=current_time;

IF symout THEN gen_sym_file;
end_sym_gen:=current_time;

IF map THEN disp_hash;
end_map_gen:=current_time;

IF xref THEN disp_xref;
end_xref_gen:=current_time;

IF bell THEN FOR i:=1 TO 10 DO write(chr(7));
...
IF chat OR debug OR (NOT quiet) THEN
  ...
  writeln('total CPU time:- ',
	  (end_link.mill_time-start_link.mill_time)/1000:7:2);
  END;
