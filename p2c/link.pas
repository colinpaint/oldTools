...
[EXTERNAL] FUNCTION lib$get_foreign (%DESCR result,user_prompt:string;
                                     %REF out_len:word; %REF force_prompt:integer):integer; EXTERN;
{<<<}
TYPE
  ...
  nam_ptr_t = ^NAM$TYPE;
{>>>}
FUNCTION PAS$FAB (VAR F: unsafe_file_t) : fab_ptr_t; EXTERN;
...
{<<<}
VAR
  ql_undefsym, ql_dbldefsym, ql_sizeclash, ql_fatalerr, ql_sctovrlap : integer;
  ...
  line_length:integer;
  line_buff:PACKED ARRAY [1..max_len+1] of char; {extra byte to stop bound overflow!}
...
IF log THEN
  writeln (log_file, 'in module ''',mod_id,''', from file ''',cur_file,'''....');
...
FOR i := 0 TO max_hash DO
  IF hash_table[i]<>nil THEN
    ...
    REPEAT
      write (map_file,s_ptr^.sname,' ', hex (s_ptr^.saddr+baseaddr[s_ptr^.ssect], 6, 6), ' ', s_ptr^.mname);
      IF s_ptr^.comsize<>-1 THEN
...
IF english THEN
  ...
  IF sectbase[9] <> 0 THEN
    BEGIN
    writeln (log_file, 'Size of HELP              (9)  = ', sectbase[9]:8, ' bytes');
