...
{ put an error message into errortable. Halt compilation if
  ...
  begin
    ...
    if fatalflag then
      begin
      if scanalys then scan2
      else if stringtablelimit < stringfilecount + stringtabletop then scan2;
      goto 99;
      end;
  end {warnat} ;
...
{ call an overlay where caller resides at the same overlay level
  ...
  the work.  This approach works for all three major DEC operating
  systems.
}
...
begin {main}
  ...
  if (targetopsys = unix) and (targetmachine = mc68000) then
    if unixtarget = NEC then
      p_nec;                   { NEC license check }
