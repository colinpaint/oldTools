...
procedure callo(procedure p {procedure to call} );
...
procedure p2readreal(function getch(first: boolean): char;
                     var result: realarray;
...
procedure set_default_ext(var res: packed array [lo..hi: natural] of char;
                          kind: file_kind {kind of file needed} );
  external;
...
procedure init_text(var base: array [lo..hi: natural] of stringptr);
 { initialize a text table }
  external;


procedure disp_text(var base: array [lo..hi: natural] of stringptr);
...
function get_char(where: stringindex; {location of char}
                  var base: array [lo..hi: natural] of stringptr): char;
...
procedure put_text(where: stringindex; {starting character}
                   var base: array [lo..hi: natural] of stringptr;
                   var sample: string_buffer {data to store} );
...
function match_text(start: stringindex; {starting character}
                    ...
                    var base: array [lo..hi: natural] of stringptr;
                    var sample: string_buffer {name to match} ): boolean;
...
function debughash(len: integer;
                   function nextchar: char): integer;
...
function opendebugfile(var filename: FilenameBuf;
                       ext: packed array
 [elower..eupper: integer] of char): boolean;

  external;


procedure closedebugfile(function nextblock
                              (var blk: dbgstringblock): boolean {is there more}
                         );
