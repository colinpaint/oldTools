{$s}
{TEST 6.4.3.5-12, CLASS=CONFORMANCE}

{: This test contains six simultaneous open files. }
{  The Standard permits any number of file variables;
   this test writes to six files simultaneously to check
   that any limit imposed by an implementation is not
   ridiculously small. }
{V3.1: New test. }

program t6p4p3p5d12(output);
const
   Limit=47;
type
   FourCycle = (P,Q,R,S);
...
begin
   ...
   if result=pass then
      writeln(' PASS...6.4.3.5-12')
   else
      writeln(' FAIL...6.4.3.5-12')
end.
