{$s}
{TEST 6.6.3.3-6, CLASS=CONFORMANCE}

{: This test checks that non-text file parameters are accepted. }
{  This test explores the use of files of type other than text
   (or file of char) as var parameters to establish that they are
   implemented.  Only local files can be tested; whether external files
   can be of these types is implementation-dependent (and may be
   omitted from an implementation therefore). }
{V3.1: New test. }

program t6p6p3p3d6(output);
var
   ix : integer;
   result : (pass,fail);
...
begin
   ...
   Unions;
   if result=pass then
      writeln(' PASS...6.6.3.3-6')
   else
      writeln(' FAIL...6.6.3.3-6')
end.
