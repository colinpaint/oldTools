{TEST 6.9.1-6, CLASS=QUALITY}

{: This test checks the accuracy of read for reals. }
{  Real values which can be exactly represented in the machine
   ...
   for I = 1 .. 100 to a temporary file in decimal as
   characters strings and then reads them back, checking for
...
{V3.1: Program parameter removed and number of items
   reduced from 1000 to 100. Machar changed. }

program t6p9p1d6(output);
const
   items = 100;
var
   tempfile: text;
...
var
   ...
   procedure writerfract (exp: integer);
   { writes one line with rfract using given exponent }
   var
      i: integer;
   begin
      for i := 1 to exp do
         write(tempfile, rreg [i]);
      write(tempfile, '.');
      for i := exp + 1 to length do
         write(tempfile, rreg [i]);
      i := exp;
      writeln(tempfile, 'E-', chr ((i div 10) + ord ('0')), chr (
         (i mod 10) + ord ('0')))
   end { writerfract };
...
begin { readfile }
   ...
   writeln(' TEST ACCURACY OF READ FOR REALS');
   printresults
end { readfile };
...
begin { Main program }
   ...
   reset(tempfile);
   readfile;
   writeln(' OUTPUT FROM TEST...6.9.1-6');
   if support
   then
      writeln(' READ OF REALS SUPPORTED IN SENSE OF W S BROWN')
   else
      writeln(' READ OF REALS IS APPROXIMATE');
   if failed
   then
      writeln(' FAIL...6.9.1-6')
   else
      writeln(' QUALITY...6.9.1-6')
end.
